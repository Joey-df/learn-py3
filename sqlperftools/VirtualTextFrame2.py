...
class VirtualTextFrame(ttk.Frame):
    ...
    def create_widgets(self):
        ...
        control_frame.columnconfigure(1, weight=1)  # 让搜索框可以扩展

        # 分页控制
        ttk.Label(control_frame, text="分页大小:").grid(row=0, column=0, padx=(0, 5))
        self.page_size_var = tk.StringVar(value=str(self.page_size))
        page_size_combo = ttk.Combobox(control_frame, textvariable=self.page_size_var,
                                       values=["100", "500", "1000", "5000"], width=8)
        page_size_combo.grid(row=0, column=1, padx=(0, 10))
        ...
        self.first_btn = ttk.Button(btn_frame, text="首页", command=self.goto_first_page, width=6)
        self.first_btn.grid(row=0, column=0, padx=2)

        self.prev_btn = ttk.Button(btn_frame, text="上一页", command=self.goto_prev_page, width=6)
        self.prev_btn.grid(row=0, column=1, padx=2)

        self.next_btn = ttk.Button(btn_frame, text="下一页", command=self.goto_next_page, width=6)
        self.next_btn.grid(row=0, column=2, padx=2)

        self.last_btn = ttk.Button(btn_frame, text="末页", command=self.goto_last_page, width=6)
        self.last_btn.grid(row=0, column=3, padx=2)

        # 跳转页面
        ttk.Label(control_frame, text="跳转到:").grid(row=0, column=3, padx=(10, 5))
        self.goto_var = tk.StringVar()
        goto_entry = ttk.Entry(control_frame, textvariable=self.goto_var, width=8)
        goto_entry.grid(row=0, column=4)
        goto_entry.bind("<Return>", self.goto_page)

        # 页面信息
        self.page_info = ttk.Label(control_frame, text="")
        self.page_info.grid(row=0, column=5, padx=10)

        # 搜索框
        ttk.Label(control_frame, text="搜索:").grid(row=0, column=6, padx=(20, 5))
        self.search_var = tk.StringVar()
        search_entry = ttk.Entry(control_frame, textvariable=self.search_var, width=20)
        search_entry.grid(row=0, column=7, sticky="ew", padx=(0, 5))

        # 搜索按钮
        search_btn = ttk.Button(control_frame, text="搜索", command=self.perform_search, width=6)
...
class LargeDataApp:
    def __init__(self, root):
        self.root = root
        self.root.title("大数据量Text组件优化方案")
        ...
        self.virtual_text.grid(row=0, column=0, sticky="nsew")
...
"""
        ...
        # 创建滚动文本区域
        text_container = ttk.Frame(self.info_frame)
        ...
        info_widget.grid(row=0, column=0, sticky="nsew")
...
"""
        ...
        text_label.grid(row=0, column=0, sticky="w", pady=(0, 20))
        ...
        conclusion_frame.grid(row=2, column=0, sticky="ew", pady=20)
